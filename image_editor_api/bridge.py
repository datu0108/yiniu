...
def _prepare_upload(data: bytes, name: str) -> tuple[bytes, str]:
    ...
    try:
        ...
        while quality > 40:
            ...
            if len(out_data) <= target_bytes:
                return out_data, "image/jpeg"
            quality -= 10
...
def cmd_batch_retry(batch_id: str, delay_seconds: float = 1.0) -> int:
    """重新运行批量任务中失败的任务。"""
    batch_dir = BATCHES_DIR / batch_id
    batch_path = batch_dir / "batch.json"
    if not batch_path.is_file():
        _emit({"ok": False, "error": f"批量任务不存在: {batch_id}"})
        return 1

    batch_meta = json.loads(batch_path.read_text(encoding="utf-8"))
    prompt = batch_meta["prompt"]
    output_dir = Path(batch_meta["output_dir"])
    
    new_jobs = []
    retried_count = 0
    
    for entry in batch_meta["jobs"]:
        job_id = entry["job_id"]
        st = _read_status(JOBS_DIR / job_id)
        if st.get("status") == "error":
            # 重起任务
            image_path = Path(entry["input_path"])
            new_job_id = _start_background_job(image_path, prompt, output_dir)
            new_jobs.append({
                "job_id": new_job_id,
                "input_path": str(image_path),
                "status": "running"
    ...
    if retried_count == 0:
        _emit({"ok": True, "message": "没有失败的任务需要重试", "batch_id": batch_id})
        return 0
        
    batch_meta["jobs"] = new_jobs
    batch_meta["updated_at"] = time.time()
    batch_path.write_text(json.dumps(batch_meta, ensure_ascii=False, indent=2), encoding="utf-8")
    
    _emit({"ok": True, "message": f"已重试 {retried_count} 个失败任务", "batch_id": batch_id})
    return 0


def cmd_list_batches(limit: int = 10) -> int:
    """列出最近的批量任务。"""
    if not BATCHES_DIR.is_dir():
        _emit({"ok": True, "batches": []})
        return 0
        
    batches = []
    for bdir in sorted(BATCHES_DIR.iterdir(), key=lambda x: x.stat().st_mtime, reverse=True):
        if not bdir.is_dir(): continue
        bp = bdir / "batch.json"
        if bp.is_file():
            meta = json.loads(bp.read_text(encoding="utf-8"))
            summary = _summarize_batch(meta["batch_id"])
            batches.append({
                "batch_id": meta["batch_id"],
                "created_at": meta.get("created_at"),
                "status": summary.get("status"),
                "progress": f"{summary.get('done')}/{summary.get('total')}",
                "prompt": meta.get("prompt")[:50] + "..." if len(meta.get("prompt", "")) > 50 else meta.get("prompt")
            })
        if len(batches) >= limit:
            break
            
    _emit({"ok": True, "batches": batches})
    return 0
...
def main() -> int:
    ...
    batch_retry_p = sub.add_parser("batch-retry", help="重试批量任务中的失败项")
    batch_retry_p.add_argument("--batch", required=True)

    list_batches_p = sub.add_parser("list-batches", help="列出历史批量任务")
    list_batches_p.add_argument("--limit", type=int, default=10)
    ...
    if args.cmd == "batch-retry":
        return cmd_batch_retry(args.batch)
    if args.cmd == "list-batches":
        return cmd_list_batches(args.limit)
